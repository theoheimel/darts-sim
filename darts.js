{
    ...
    const makeTexture = pixels => regl.framebuffer({
        color: regl.texture({
            width: pixels,
            height: pixels,
            format: 'rgba',
    ...
    const output = makeTexture(textureSize);
    ...
    const updateSliderValues = () => {
        for (let id of sliderIds) {
            const sval = parseFloat(sliders[id].value);
            if (sval != sliderValues[id]) {
                sliderValues[id] = sval;
                slidersChanged = true;
            }
        }
    };

    let xoffSlider = document.getElementById('xoff');
    let yoffSlider = document.getElementById('yoff');
    let sigmaSlider = document.getElementById('sigma');
    let excentricitySlider = document.getElementById('excentricity');
    ...
    regl.frame(() => {
        const newSliderValue = parseFloat(sigmaSlider.value);
        updateSliderValues();
